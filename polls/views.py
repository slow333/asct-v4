...
def create_poll(request):
    if request.method == 'POST':
        ...
        q_form = QuestionForm()
        c_form = ChoiceForm()
        return render(request, 'polls/polls_form.html', 
...
def update_poll(request, pk):
    ...
    if request.method == 'POST':
        ...
        if q_form.is_valid() and formset.is_valid():
            ...
            return redirect('blog:detail', pk=question.blog.id ) # type: ignore
    else:
...
def vote(request, pk):
    ...
    try:
        ...
        if question.blog:
            return redirect(reverse('blog:detail', kwargs={'pk': question.blog.id}) + f'?voted_pk={question.id}') # type: ignore
