...
class VenueFormAdmin(forms.ModelForm):
    owner = forms.ModelChoiceField(
        ...
        required=True,
        label=mark_safe(
            '<span style="display: inline-block; font-size: 1rem; margin: 10px; color: green;">장소 주인</span>'
        ),
        empty_label="장소 주인 선택",
        widget=forms.Select(
            attrs={"class": "form-select", "placeholder": "장소 주인 선택"}
        ),
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        for field in self.fields:
            ...
            self.fields[field].widget.attrs.update(
                {"placeholder": f'{field.replace("_", " ").capitalize()}'}
            )
    ...
    class Meta:
        ...
        widgets = {
            "name": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "장소 이름"}
            ),
            "address": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "주소"}
            ),
            "web": forms.URLInput(
                attrs={"class": "form-control", "placeholder": "웹사이트"}
            ),
        }
...
class EventFormAdmin(forms.ModelForm):
    venue = VenueChoiceField(
        ...
        label="",
        empty_label="장소 선택",
        widget=forms.Select(attrs={"class": "form-select", "placeholder": "장소 선택"}),
    )
    ...
    class Meta:
        ...
        fields = [ "title", "description", "start_date", "end_date",
            "is_completed", "venue", "manager", "attendees",
        ]
        labels = {
            "title": "",
            "start_date": "시작일시",
